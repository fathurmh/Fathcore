    ...
    internal sealed class Helper : IHelper
    {
        ...
        public string GenerateRandomString(int length)
        {
            ...
            return new string(Enumerable.Repeat(_chars, length)
                .Select(s =>
                {
                    var rand = _random.Next(s.Length);
                    var flagCase = rand < s.Length / 2;
                    return flagCase ? char.ToUpper(s[rand]) : char.ToLower(s[rand]);
                })
                .ToArray());
        }
        ...
        public IEnumerable<string> GetTextWithinQuotes(string text)
        {
            MatchCollection vi = _extractTextDoubleQuoted.Matches(text);

            return _extractTextDoubleQuoted.Matches(text).OfType<Match>().Select(p => p.ToString());
        }
        ...
        public string RemoveSpecialCharacters(string text, params char[] except)
        {
